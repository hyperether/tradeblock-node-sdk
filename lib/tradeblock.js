...
var _constrructSignature = function(msg, params, secret){
  ...
  msg = msg.replace(/ /g, '+').replace(/,/g,'%2C');
  return crypto.createHmac('SHA256', secret).update(msg).digest('hex');
}

var _runRequest = function(){
  running = true;
  var requestData = requestQueue.shift();
  Request[METHODS[requestData.method]](requestData.request_options, function(error, response, body){
    requestData.callback(error, response, body)
    if (requestQueue.length > 0) _runRequest();
    else running = false;
  });
}

var _addToQueue = function(nonce, method, request_options, callback) {
  requestQueue.push({nonce, method, request_options, callback});
  if (!running) _runRequest();
}
...
TradeBlock.prototype.request = function(base, method, params) {
  ...
  return new Promise(function(resolve, reject){
    ...
    _addToQueue(nonce, method, request_options, function(error, response, body){
